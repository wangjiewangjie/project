...
export default {
  ...
  setShareInfo(
    ...
    shareInfo.copyLink = 'http://www.baidu.com';
  ...
  setShareQuery(params) {
    ...
    if (params.shareId === undefined) {
      if (commonInfo.getToken()) {
        shareUserId = storage.get('user').id;
      }
    }
    ...
    let platform = ['H5', 'wxOfficialAccount', 'wxMiniProgram', 'App'].indexOf($platform.get()) + 1; // 设置分享的平台渠道: 1=H5,2=微信公众号网页,3=微信小程序,4=App,...按需扩展
  ...
  getShareQuery(spm) {
    ...
    if (shareParamsArray[1] === '1') {
      //// 默认首页不跳转
      return;
    } else if (shareParamsArray[1] === '2') {
      shareParams.page = '/pages/goods/goods/index';
      return;
    }
